...
class SOT_with_DRL_Tr:
    ...
    def Train(self):
        ...
        for i_episode, subfolder in enumerate(frcnn_folders):
            if i_episode >= 7:
                break
            ...
            initial_image_index = 0  # 50 프레임 단위로 반복학습
            while initial_image_index is not None:
                ...
                for i in range(person_num):
                    ...
                    while not done and current_img_indx is not None:
                        ...
                        # 첫 프레임: i번째 사람을 GT로 고정해 SORT 시작
                        if current_img_indx == initial_image_index:
                            dets_first = np.empty((0, 6))
                            for det in pred:
                                if det is not None and len(det):
                                    for x1, y1, x2, y2, conf, detclass in det.cpu().detach().numpy():
                                        if int(detclass) == 0:
                                            dets_first = np.vstack(
                                                (dets_first, np.array([x1, y1, x2, y2, conf, detclass]))
                                            )
                            if dets_first.shape[0] == 0:
                                print("No person on first frame. skip.")
                                break
                            ...
                            if len(kalman_states) > 0:
                                ...
                                if hasattr(tracker, 'get_state'):
                                    # 1) 속도/가속도/각속도는 tracker에서 그대로 가져오고
                                    ...
                                    # 2) 위치 정보(x1,y1,x2,y2)는 i번째 검출 박스 그대로 사용
                                    x1, y1, x2, y2 = dets_first[i][0:4]

                                    # 3) convert_bbox_format 이 기대하는 포맷대로 넣어줌
                                    bbox_state = np.array([x1, y1, x2, y2, *vel, *acc, angv],
                                                        dtype=np.float32).reshape(1, 9)
                                    temp = convert_bbox_format(bbox_state)

                                    track_id = int(kalman_predict[0, 8])
                                    self._log_frame(log_file, cur_img_path.name, track_id, dets_first[i][:4], vel, acc, angv)

                                    # 좌표 복원 + 저장
                                    box = dets_first[i, 0:4].copy().reshape(1, 4)
                                    box = scale_coords(im.shape[1:], box, img0.shape, ratio_pad=(ratio, pad)).round()
                                    img_draw = draw_boxes(
                                        img0.copy(),
                            ...
                            # 이후 프레임: 검출 결과 중 SORT 예측과 IOU 최대 박스 선택
                            max_iou = 0.1
                            best_target = None
                            ...
                            for det in pred:
                                ...
                                for x1, y1, x2, y2, conf, detclass in det.cpu().detach().numpy():
                                    if int(detclass) != 0:
                                        continue
                                    # 주의: kalman_predict는 (cx,cy,h,w, ...). 여기선 기존 코드 유지
                                    cx, cy, h, w = kalman_predict[0][:4]
                                    # 원 코드는 여기서 [cx,cy,h,w] vs [x1,y1,x2,y2]로 iou를 계산했지만
                                    # 기존 동작을 그대로 따르기 위해 유지한다.
                            ...
                            if best_target is not None:
                                ...
                                if any(val in (0, 640) for val in [cx, cy, h, w]):
                                    ...
                                    if len(kalman_states) > 0:
                                        ...
                                        if hasattr(tracker, 'get_state'):
                                            ...
                                            track_id = int(kalman_predict[0, 8])
                                            self._log_frame(log_file, cur_img_path.name, track_id, best_target[0, :4], vel, acc, angv)
                                            ...
                                            if temp is not None:
                                                ...
                                                # 🔹 best_target은 여기서 항상 존재하므로, ID 일치 여부만 체크
                                ...
                                # best_target 없음 → 현재 프레임 이미지라도 저장(원 코드 유지)
                                img_draw = img0.copy()
                                self._save_frame(log_dir / f'img1_{i}', cur_img_path.name, img_draw)
                                ...
                                if prev_track_id is not None and state is not None:
                                    ...
                                    if next_state is not None:
                                        # 이 프레임에 도달하기 위해 사용한 frame rate = self.currentFr
                                        # 그에 해당하는 action index 를 찾는다
                                        try:
                                            action_idx = self.Agent.Frame_Rates.index(self.currentFr)
                                        except ValueError:
                                            action_idx = 0  # 혹시 리스트에 없으면 기본값
                                ...
                                # ===== 실패인 경우에는 다음 프레임은 항상 30fps로 강제 =====
                                predicted_fr = 30
                                self.currentFr = 30
                                if 30 in fr_count:
                                    fr_count[30] += 1

                                # 30fps 기준으로 다음 프레임 인덱스 계산
                                current_img_indx = get_next_frame_index(current_img_indx, predicted_fr, total_img_num)
                ...
                # 50 프레임 단위로 다음 블록
                initial_image_index = initial_image_index + 50 if (initial_image_index + 50) < len(image_files) else None
