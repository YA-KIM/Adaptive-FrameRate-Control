        ...
        for i_episode, subfolder in enumerate(frcnn_folders):
           ...
           # === log file settings ===
            ...
            while not done and current_img_index is not None:
                ...
                if len(dets_to_sort):
                    dets_to_sort[:, :4] = scale_coords(img.shape[1:], dets_to_sort[:, :4], img0.shape, ratio_pad=(ratio, pad))
                ...
                for tracker in kalman_states:
                    if hasattr(tracker, 'get_state'):
                        ...
                        log_file_eval.write(f"{frame_id},{track_id},{x1:.2f},{y1:.2f},{x2-x1:.2f},{y2-y1:.2f},1\n")
