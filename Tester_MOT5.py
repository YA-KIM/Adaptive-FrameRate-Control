...
import sys
# YOLOv5 관련 import
#sys.path.insert(0, '/home/hyhy/Desktop/SYD_DtoS/DRL_FR/yolov5')
from yolov5.models.common import DetectMultiBackend, C3
...
class Options:
    # 모델 및 경로 관련
    weights: str = '/home/hyhy/Desktop/SYD_DtoS/DRL_FR/yolov5/yolov5s.pt'          # 모델 경로
    ...
    iou_thres: float = 0.1                                 # NMS에서 IOU 임계값
        ...
        for i_episode, subfolder in enumerate(frcnn_folders):
           ...
           # === log file settings ===
            ...
            while not done and current_img_index is not None:
                ...
                img, ratio, pad = letterbox(img0, new_shape=(640,640))  # 이미지 리사이즈
                print("ratio:", ratio)
                print("pad:", pad)
                ...
                img = img.half() if self.opt.half else img.float()
                img /= 255.0
                if img.ndimension() == 3:
                ...
                with torch.no_grad():
                    ...
                    pred = self.yolo_model(img, augment=self.opt.augment)
                    pred = non_max_suppression(pred, self.opt.conf_thres, self.opt.iou_thres, classes=self.opt.classes, agnostic=self.opt.agnostic_nms)

                    dets_to_sort = get_person_only(pred)
                    if dets_to_sort.size > 0:
                        coords = torch.tensor(dets_to_sort[:, :4], dtype=torch.float32, device=self.device )
                        coords = scale_coords(
                            img.shape[-2:],    # (H_resized, W_resized)
                            coords,                    
                            img0.shape[:2],            # (H0, W0)
                            ratio_pad=(ratio, pad)
                        )
                        dets_to_sort[:, :4] = coords.cpu().numpy()
                ...
                for tracker in kalman_states:
                    if hasattr(tracker, 'get_state'):
                        ...
                        box = [int(x1), int(y1), int(x2), int(y2)]  # (x1, y1, x2, y2) 형식으로 변환
                        boxes.append(box)
...
def main():
    ...
    opt = Options(source=str(dataset_path))
    ...
    agent = Agent(load=True, n_actions=4, device=device)
    tester = SOT_with_DRL_Test(agent, dataset_path, model, opt)
    # === 학습 시작 ===
    print("평가 시작...")
    tester.Test_MOT()
    print("평가 완료!")
