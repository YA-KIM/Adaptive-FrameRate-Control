...
from yolov7_object_tracking.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from yolov7_object_tracking.utils.torch_utils import torch_distributed_zero_first
...
class LoadImage:  # For inference with single image
    def __init__(self, path, img_size=640):
        p = str(Path(path).absolute())
        if os.path.isfile(p):
            self.file = p
        else:
            raise Exception(f'ERROR: {p} is not a valid file')
        ...
        if ext not in img_formats:
            raise Exception(f'ERROR: Unsupported image format: {ext}. Supported formats: {img_formats}')

    def load(self):
        im0 = cv2.imread(self.file)  # BGR
        if im0 is None:
            raise Exception(f'ERROR: Unable to load image at {self.file}')

        # Letterbox resize (maintain aspect ratio, pad as needed)
        img = letterbox(im0, self.img_size, stride=32, auto=True)[0]
        ...
        return self.file, img, im0, self.cap
