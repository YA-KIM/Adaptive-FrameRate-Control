...
def draw_boxes(img, bbox, identities=None, names=None, velocities=None, accelerations=None, angular_velocities=None, sim=None, save_with_object_id=False, path=None,offset=(0, 0)):
    for i, box in enumerate(bbox):
        ...
        text_y = y1 - 5  # 기본적으로 바운딩 박스 위쪽에 표시

        if text_y - h < 0:  # 텍스트가 위로 넘어가는 경우
            text_y = y1 + h + 5  # 바운딩 박스 아래로 이동
...
def sot(save_img=False):
    ...
    for filename in possible_gt_files:
        ...
        if gt_paths.exists():
            gt_path = gt_paths
            break
    target_track_id = None  # 추적할 객체의 ID
    feature_maps = []
    ...
    sort_tracker = Sort(max_age=10, min_hits=1, iou_threshold=0.3)
    ...
    with open(log_file_path, 'w') as log_file:
        ...
        fail=0
        max_iou = 0
        max_similarity1 = 0.99
        max_similarity2 = 0.998
        log_file.write(f"{max_similarity1}/{max_similarity2}\n")
        for path, img, im0s, vid_cap in dataset:
            max_sim1 =max_similarity1
            max_sim2 = max_similarity2
            image_count += 1
            ...
            if frame_count < 10:  # 초기 프레임 처리
                if len(initial_detections) > frame_count:
            ...
            Max_len = 1
            max_similarity=max_sim1
            best_target = None
            if frame_count >= 10:  # 이후 프레임 처리
                matched=False
                if len(detected_boxes) is not None:
                    print(f"SORT tracked detections: {tracked_dets}")
                    ...
                    for saved_feature in initial_features:
                        for det in tracked_dets:
                            ...
                            if similarity > max_similarity:
                                ...
                                if iou > max_iou:
                                    max_iou=iou
                                    best_target = (x1, y1, x2, y2, similarity, iou)
                                    matched=True
                            else : 
                                break

                    if best_target:
                        x1, y1, x2, y2, similarity, iou = best_target
                        conf=1
                        cls=0
                        print(f"Best target found: {x1, y1, x2, y2} | Similarity: {similarity:.3f} | IoU: {iou:.3f}")
                        current_center = ((x1 + x2) // 2, (y1 + y2) // 2)
                        update_object_info(object_info, track_id, current_center)
                ...
                if len(detected_boxes)==0 or matched==False :
                    pred = model(img, augment=opt.augment)[0]
                    pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)
                    max_similarity = max_sim2
                    target_det = None

                    if pred[0] is not None:  # YOLO 결과가 있을 때
                        ...
                        for det in pred[0]:
                            ...
                            if similarity > max_similarity:
                                max_similarity = similarity
                                target_det = (x1, y1, x2, y2, conf, roi_feature)

                    if target_det:
                        x1, y1, x2, y2, conf, roi_feature = target_det
                        ...
                        print("YOLO also failed to find a matching target.")
                        fail+=1
...
if __name__ == '__main__':
    source_path = '/home/hyhy/Datasets/FR_Dataset/LaSOT/bird/bird-1/img'
    ...
    parser.add_argument('--project', default='/home/hyhy/Desktop/SYD_DtoS/DRL_FR/yolov7_object_tracking/runs/sot', help='project save folder')
