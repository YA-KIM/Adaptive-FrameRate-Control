...
def get_next_frame_index(current_index, fps, total_images):
    fps_map = {60: 1, 30: 2, 20: 3, 10: 6}
    increment = fps_map.get(fps, 1)  # 기본값 1

    next_index = current_index + increment

    return None if next_index >= total_images else next_index
...
def detect(source=None, opt=None):
    ...
    for filename in possible_gt_files:
        gt_paths = Path(opt.source).parent / filename
        if gt_paths.exists():
            gt_path = gt_paths
            break
    feature_maps = []
    ...
    log_file_path = save_dir / f"{opt.name}.txt"  # 저장 파일 경로

    initial_detections = []
    import os
    print("Real Path:", os.path.realpath(opt.source))
    ...
    with open(log_file_path, 'w') as log_file:
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                prev_best_target = best_target if best_target else None
                ...
                if tracked_dets.size == 0:
                    print("No target detected in the current frame.")
                    if best_target:
                        x1, y1, x2, y2, _, _ = best_target  # best_target 값 가져오기
                        x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)  # 정수 변환
                        cv2.rectangle(im0s, (x1, y1), (x2, y2), (255, 0, 0), 2)
                        cv2.putText(im0s, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                        save_path = save_dir / f"{img_name}.jpg"
                        cv2.imwrite(str(save_path), im0s)
                        print(f"Frame saved to {save_path}")
                        log_file.write(f"Frame: {img_name}, No detection\n")
                    else: 
                        log_file.write(f"Frame: {img_name}, No detection\n")
                        save_path = save_dir / f"{img_name}.jpg"
                        cv2.imwrite(str(save_path), im0s)
                ...
                tracked_ids = set(identities)  # 현재 프레임에 존재하는 ID들
                for track_id in list(object_lifetime.keys()):
    ...
    return tracked_dets, tracks

def detect_frame(model, device, half, source=None, opt=None, initial_features=None):
    source, weights, view_img, save_txt, imgsz, trace, colored_trk, save_bbox_dim, save_with_object_id, fps = (
        opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, 
        not opt.no_trace, opt.colored_trk, opt.save_bbox_dim, opt.save_with_object_id, opt.fps
    )
    ...
    gt_path = None
    for filename in possible_gt_files:
        gt_path = Path(opt.source).parent / filename
        if gt_path.exists():
            break
    feature_maps = []
    save_img = not opt.nosave and not source.endswith('.txt')
    ...
    for path, img, im0s, vid_cap in dataset:
        with open(log_file_path, 'a') as log_file:
            ...
            if int(img_name) < 5:  # 초기 프레임 처리
                ...
                if not initial_detections:
                    print(f"No detection found for frame {img_name}. Setting default bbox.")
                    target_bbox = np.array([0, 0, 0, 0, 0])  # 기본값 설정
                else:
                    target_bbox = np.array(initial_detections[0], dtype=float)  # 🔥 NumPy 배열로 변환
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if tracked_dets.size == 0:
                    ...
                    if best_target:
                        ...
                        save_path = save_dir / f"{img_name}.jpg"
                        cv2.imwrite(str(save_path), im0s)
                        print(f"Frame saved to {save_path}1")
                        log_file.write(f"Frame: {img_name}, No detection\n")
                    else: 
                        log_file.write(f"Frame: {img_name}, No detection\n")
                        save_path = save_dir / f"{img_name}.jpg"
                        cv2.imwrite(str(save_path), im0s)
                        print(f"Frame saved to {save_path}2")
                    continue
        ...
        total_images = len(list(Path(source).parent.glob("*.jpg")))  
        current_index = get_next_frame_index(current_index, fps, total_images)
        return tracked_dets, tracks, initial_features

    print(f"Results saved to {log_file_path}")
    print(f"Done. ({time.time() - t0:.3f}s)")

if __name__ == '__main__':
    source_path = '/home/hyhy/Datasets/FR_Dataset/LaSOT/airplane/airplane-2/img'
    ...
    parser.add_argument('--fps', type=int, default=10, help='Frames per second for image sampling (60, 30, 20, 10)')
    ...
    parser.add_argument('--project', default='/home/hyhy/Desktop/SYD_DtoS/DRL_FR/yolov7_object_tracking/runs/detect_and_track2', help='project save folder')
    ...
    device = select_device(opt.device)
    half = device.type != 'cpu'  # CUDA라면 Half Precision 사용

    model = attempt_load(opt.weights, map_location=device)  # 모델 로드
    stride = int(model.stride.max())  # 모델 stride
    opt.img_size = check_img_size(opt.img_size, s=stride)  # 이미지 크기 확인
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            ...
            detect_frame(model, device, half, source_path, opt, initial_features)
