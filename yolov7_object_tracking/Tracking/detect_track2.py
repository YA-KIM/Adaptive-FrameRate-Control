        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            if frame_count >= 5:  # 이후 프레임 처리  
                    ...
                    for saved_feature in initial_features:
                        for det in pred[0]:
                            ...
                            if len(detected_boxes) is not None:
                                #print([x1, y1, x2, y2], detected_boxes[-1][:4])
                                iou = calculate_iou([x1, y1, x2, y2], detected_boxes[-1][:4])
 
                            if matched == True: 
                                if iou > max_iou and similarity > max_similarity and iou< 1:
                                    max_iou = iou
                ...
                if best_target:
                    ...
                    print(f"Target found by YOLO: {x1, y1, x2, y2} | Similarity: {max_similarity:.3f} | IOU: {max_iou:.3f}")
                    current_center = ((x1 + x2) // 2, (y1 + y2) // 2)
                    update_object_info(object_info, track_id, current_center)
                    if track_id in object_info:
                        velocity = object_info[track_id]['velocities'][-1] if object_info[track_id]['velocities'] else (0, 0)
                    ...
                    if len(detected_boxes) is not None:
                        ...
                        if best_target:
                            x1, y1, x2, y2, similarity, iou = best_target
                            print(f"Target found by SORT: {x1, y1, x2, y2} | Similarity: {max_similarity:.3f} | IOU: {max_iou:.3f}")
                            current_center = ((x1 + x2) // 2, (y1 + y2) // 2)
                            update_object_info(object_info, track_id, current_center)
                            if track_id in object_info:
                                velocity = object_info[track_id]['velocities'][-1] if object_info[track_id]['velocities'] else (0, 0)
                            ...
                            #print("SORT also failed to find a matched object")
                            matched = False
                            fail += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='/home/hyhy/Desktop/SYD_DtoS/DRL_FR/yolov7_object_tracking/runs/sot2', help='project save folder')
