...
from yolov7_object_tracking.models.experimental import attempt_load
from yolov7_object_tracking.utils.datasets import LoadStreams, LoadImages, LoadImage
from yolov7_object_tracking.utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, apply_classifier, \
                scale_coords, xyxy2xywh, strip_optimizer, set_logging
from yolov7_object_tracking.utils.plots import plot_one_box
from yolov7_object_tracking.utils.torch_utils import select_device, load_classifier, time_synchronized, TracedModel
from yolov7_object_tracking.utils.download_weights import download

#For SORT tracking
import skimage
from sort import *
from yolov7_object_tracking.models.yolo import extract_roi_feature, visualize_roi, compute_similarity, get_histogram, compute_histogram_similarity
...
def detect_frame(model, device, half, opt: Options, source=None, gt_folder=None, initial_features=None, prev_best_target=None, sort_tracker =None, first_hist=None, gt_format = None, frame_rate =60):
    ...
    if webcam:
        ...
        dataset = LoadImage(source, img_size =imgsz)
    else:
    ...
    with torch.no_grad():
        ...
        object_lifetime = {}
        path, img, im0s,_ = dataset.load()    #img = 전처리 되는 이미지 (for Yolo) / im0s = 완전한 원본 이미지
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if int(img_name) <= 5:  # 초기 프레임 처리
                ...
                # bbox가 numpy일 수 있으니 명확히 튜플로 변환
                bbox = tuple(map(int, np.array([x1, y1, x2, y2]).flatten()))
                roi_feature = extract_roi_feature(im0s, bbox)
                ...
                if len(det):
                    det[:, :4] = scale_coords(img.shape[2:], det[:, :4], im0.shape).round()
                    for obj in det.cpu().detach().numpy():
                        x1, y1, x2, y2, conf, cls = map(int, obj)
                        ...
                        candidates.append({
                            ...
                            'hist_sim': hist_sim,
                            'feat_sim': max_similarity
                        })

                    if candidates:
                        # color 상위 3개
                        sorted_by_color = sorted(candidates, key=lambda x: x['hist_sim'], reverse=True)
                        for i, c in enumerate(sorted_by_color[:3]):
                            c['color_rank'] = i + 1
                        
                        # feature 상위 3개
                        sorted_by_feat = sorted(candidates, key=lambda x: x['feat_sim'], reverse=True)
                        for i, c in enumerate(sorted_by_feat[:3]):
                            c['feat_rank'] = i + 1

                        # 순위 합산으로 best_target 결정
                        best_score = float('inf')
                        #print(candidates)
                        for c in candidates:
                            color_rank = c.get('color_rank', 8)
                            feat_rank = c.get('feat_rank', 8)
                            total_score = color_rank + feat_rank

                            if total_score < best_score:
                                best_score = total_score
                                best_feat_rank = feat_rank
                                best_target = c['box']
                            elif total_score == best_score:
                                if feat_rank < best_feat_rank:
                                    best_feat_rank = feat_rank
                                    best_target = c['box']
